...
BRSb = ['H3B101A_13250', 'H3B203J_13380', 'H4B204J_13340', 'LDX55_06330']
...
color_dict = {'GS3': '#FF7594', 'GS4': '#FF8A75', 'GS1': '#FF7575', 
              ...
              'S3': '#FFF575', 'BRSb': '#D375FF'} #Dictionary to color nodes by gene type

color_edge_dict = {'GS3': '#C96279', 'GS4': '#C96D5C', 'GS1': '#C95E5E',  
                   'GS2': '#C95F91', 'BRS': '#B95ECB', 'NGB': '#995CC6', 
                   'S1': '#C97F5D', 'S2a': '#C9925E', 'S2b': '#C9A95C', 
                   'S3': '#C9C15C', 'BRSb': '#A85ECB'} #Dictionary to color edges by gene type
...
for GH_type in GH_types: #Loop through GH families (GH70/32)
    ...
    colors = [color_dict[tag_dict[loctag]] if loctag not in BRSb else color_dict['BRSb'] for loctag in node_names] #Get the colors of each node
    
    tag_dict = {k:(v if k not in BRSb else 'BRSb') for (k, v) in tag_dict.items()} #Change the type of some BRS to BRSb
