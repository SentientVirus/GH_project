...
PTL = ['A0901_14250', 'A1001_13210', 'A1003_13400', 'A1202_14500',
       'A1401_13710', 'A1404_14290', 'A1805_13670', 'K2W83_RS06625',
       'FHON2_14510', 'G0101_13740', 'G0403_13980', 'H1B104J_13940',
       'H1B105A_13110', 'H1B302M_13810', 'H3B101A_14180', 'H3B104J_13910',
       'H3B104X_14110', 'H3B202X_13780', 'H3B203J_14310', 'H3B203M_13420',
       'H3B206M_13730', 'H3B209X_14340', 'H4B111J_14460', 'H4B202J_13700',
       'H4B204J_14250', 'H4B205J_13920', 'H4B206J_14400', 'H4B211M_13940',
       'H4B402J_13530', 'H4B405J_14150', 'H4B406M_14380', 'H4B412M_14170',
       'H4B501J_13790', 'H4B503X_13610', 'H4B504J_14270', 'H4B505J_13820',
       'LDX55_06780', 'APS55_RS03400']

PTL_prev = ['A0901_14260', 'A1001_13220', 'A1003_13410', 'A1202_14510',
       'A1401_13720', 'A1404_14300', 'A1805_13680', 'K2W83_RS06630',
       'FHON2_14520', 'G0101_13750', 'G0403_13990', 'H1B104J_13950',
       'H1B105A_13120', 'H1B302M_13820', 'H3B101A_14190', 'H3B104J_13920',
       'H3B104X_14120', 'H3B202X_13790', 'H3B203J_14320', 'H3B203J_14330', 'H3B203M_13430',
       'H3B206M_13740', 'H3B209X_14350', 'H4B111J_14470', 'H4B202J_13710',
       'H4B204J_14260', 'H4B205J_13930', 'H4B206J_14410', 'H4B211M_13950',
       'H4B402J_13540', 'H4B405J_14160', 'H4B406M_14390', 'H4B412M_14180',
       'H4B501J_13800', 'H4B503X_13620', 'H4B504J_14280', 'H4B505J_13830',
       'LDX55_06785', 'APS55_RS03395']

pepN = ['K2W83_RS06615', 'LDX55_06765', 'APS55_RS03410']

all_genes = PTL + PTL_prev + pepN
...
color_dict = {'PTL': '#987284', 'pur': '#F9B5AC',
              'pepN': '#A3C4BC', 'int': '#BFD7B5'}
...
outfig = f'{projdir}/plots/pyGenomeViz/PTL_region.svg'
...
def get_info(phylo_dict, folder_path2, length = 0):
    ...
    locus_dict = {'IBH001': 'LDX55_06685', 'DSMZ12361': 'K2W83_RS06535', 
                  'MP2': 'APS55_RS03490'}
                ...
                if strain not in ['IBH001', 'MP2', 'DSMZ12361']: #Retrieve start position in the strains
                    for feature in record.features:  #Loop through features (mostly CDS) in the record
                        ...
                        if len(gene_name) > 0: #If there is at least one qualifier
                            ...
                            if 'ywnA' in gene.value: #If the gene name includes ohrR
                    ...
                    for feature in record.features: #Loop through genes in the strain
                        ...
                        if len(locus_tag) > 0 and locus_tag[0].value.replace('"', '') == locus_dict[strain]: #Get the gene with the same locus tag as in locus_dict
                            ...
                            if strain == 'MP2': #If the strain is MP2
                                start -= 5500 #Set the start to minus 5kb (forward)
...
for i in range(1, len(phylo_order.keys())+1): #Loop through strain names in the order dictionary
    ...
    for segment in track.segments: #Loop through segments in the track
        ...
        for cds in features: #Loop through CDS
            ...
            if 'gene' in cds.qualifiers.keys() or cds.qualifiers['locus_tag'][0].replace('AKU', '') in all_genes:
                if cds.qualifiers['locus_tag'][0].replace('AKU', '') in PTL:
                    gene_name = 'PTL'
                    color = color_dict[gene_name]
                elif cds.qualifiers['locus_tag'][0].replace('AKU', '') in PTL_prev:
                    color = color_dict['int']
                    gene_name = ''
                elif cds.qualifiers['locus_tag'][0].replace('AKU', '') in pepN:
                    gene_name = 'pepN'
                    color = color_dict[gene_name]
                elif 'transposase' not in cds.qualifiers['product'][0]: #If the gene is not a transposon
                ...
                gene_name = ''
                color = 'black'
            elif cds.location.end - cds.location.start > 4000 or cds.qualifiers['locus_tag'][0] in ['AKUH4B406M_14410', 'AKUH4B406M_14420', 'AKUH4B406M_14430', 'AKUH4B406M_14440', 'AKUH4B406M_14450']:
                color = '#F5C987'
            elif cds.location.end - cds.location.start > 1450:
                color = '#FFEF9E'
...
handles += [
    Line2D([], [], marker=">", color=color_dict['PTL'], label='Partial GH70', ms=20, ls = 'none'),
    Line2D([], [], marker=">", color=color_dict['pur'], label='Purine biosynthesis genes', ms=20, ls = 'none'),
    Line2D([], [], marker=">", color='#F5C987', label='Putative surface proteins', ms=20, ls = 'none'),
    Line2D([], [], marker=">", color='#FFEF9E', label='Variable genes', ms=20, ls = 'none'),
    Line2D([], [], marker=">", color=color_dict['int'], label='Putative integral membrane protein', ms=20, ls = 'none'),
    Line2D([], [], marker=">", color=color_dict['pepN'], label='Aminopeptidase', ms=20, ls = 'none'),
