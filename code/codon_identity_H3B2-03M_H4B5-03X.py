...
comparison = 'H3B2-03M_vs_H4B5-03X'
...
strains =  ['H3B2-03M', 'H4B5-03X']


gene_order = {29: 'ohrR', 28: 'yifK', 27: 'yifK2', 26: 'yhdG', 25: 'T5', 
              24: 'CDS8', 23: 'nox', 22: 'T4', 21: 'CDS7', 20: 'S2a', 
              19: 'GS1', 18: 'T3', 17: 'oppA', 16: 'CDS6', 
              15: 'T2', 14: 'S3', 13: 'CDS5', 12: 'sbnD', 11: 'CDS4', 
              10: 'CDS3', 9: 'CDS2', 8: 'epsE', 7: 'CDS1', 6: 'epsL', 
              5: 'ywqE', 4: 'ywqD', 3: 'ywqC', 2: 'T1', 1: 'tagU'}
...
def retrieve_gene_seqs(input_dir):
    ...
    Note that the files have to follow a specific naming scheme:
        ...
        to be one of the types in GH genes and the extension has to be
        faa or fna'''
    for nbr in os.listdir(input_dir):
        ...
        if 'other_genes' in input_dir and nbr.startswith('a_kunkeei') and 'mafft' not in nbr:
            ...
            check = True
        elif ('GH' in input_dir and any(gh_gene in nbr for gh_gene in GH_genes) and 'all' in nbr and 'mafft' not in nbr) and (('GH70' in input_dir and 'complete' in nbr) or ('GH32' in input_dir and 'complete' in nbr)):
            if 'GH70' not in input_dir and 'GH32' not in input_dir:
...
fig = plt.figure(constrained_layout=False, figsize=(158, 10))
...
for gene_no in range(1, len(gene_order)+1):
    ...
    if gene.startswith('T'):
        ...
        if gene_no >= 26 or gene_no == 16: # Invert arrows for genes in the forward strand
        ...
        # Otherwise, plot the comparisons as left labels (ytick labels)
            ...
            ax.set_ylabel('Comparison', fontsize = fontsize_max, 
                          family = 'Arial')
...
t2_patch = Line2D([], [], markeredgecolor = 'black', marker = 'o', markerfacecolor = 'black',
                  color = 'white', ms = 75, label = 'Transposons in strain')
