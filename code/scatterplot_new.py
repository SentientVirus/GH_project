...
# -*- coding: utf-8 -*-
"""
Created on Mon May 19 18:40:36 2025

This script creates a plot with pairwise evolutionary metrics (dS and dN),
including the average metrics of core genes between each pair of strains.
...
with plt.style.context('fivethirtyeight'):
    fig, axs = plt.subplots(3, 4, constrained_layout = False, figsize=(18, 12),
...
for i in range(len(GH_types)): #Loop through gene types
    ...
    with open(f'{pairwise_ds}/{GH}/dNdS.tsv') as gene: #Open file with dS values
        ...
        for index2, row2 in df2.iterrows(): #Loop through rows in the dataframe
            ...
            if strain1 == strain2: #If both genes are in the same strain
                ...
                dN_y = core_dn_dict[tuple(sorted((strain1, strain2)))] #Otherwise, store core dN in a dictionary
            ...
            dN_ylist.append(dN_y) #Add dN_y to the core pairwise dN list
    ...
    y2_list = [y_list[n] for n in range(len(y_list)) if y_list[n] <= 1.5]
    dN_y1_list = [dN_ylist[n] for n in range(len(y_list)) if y_list[n] <= 1.5]
    ...
    axs[0, i].set_ylim(0, 0.12) #Set limits to the y axis
    # if i == 3:
    #     # axs[0, i].set_ylim(0, 0.04) #Set limits to the y axis
    #     axs[0, i].set_xlim(0, 0.6) #Set limits to the x axis
    ...
    [axs[m, k].tick_params(axis='both', which='major', labelsize = tick_fontsize) for m in range(0, 3) for k in range(0, i+1)]
...
legend = fig.legend(handles = patches,  handlelength = 0.6, handleheight = 1.5, 
                    ...
                    frameon = False, fontsize = fontsize, bbox_to_anchor = (1.12, 1)) #Add legend to plot
