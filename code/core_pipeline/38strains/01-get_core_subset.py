...
representatives = ['A0901', 'A1001', 'A1003', 'A1202', 'A1401', 'A1404', 
                   'A1805', 'K2W83', 'fhon2', 'G0101', 'G0403', 'H1B1-04J', 
                   'H1B1-05A', 'H1B3-02M', 'H3B1-01A', 'H3B1-04J', 'H3B1-04X', 
                   'H3B2-02X', 'H3B2-03J', 'H3B2-03M', 'H3B2-06M', 'H3B2-09X', 
                   'H4B1-11J', 'H4B2-02J', 'H4B2-04J', 'H4B2-05J', 'H4B2-06J', 
                   'H4B2-11M', 'H4B4-02J', 'H4B4-05J', 'H4B4-06M', 'H4B4-12M',
                   'H4B5-01J', 'H4B5-03X', 'H4B5-04J', 'H4B5-05J', 'LDX55',
                   'MP2'] #Set of strains of interest
...
for file in faas: #Loop through protein fasta
    with open(f'{inpath}/{file}') as handle: #Open input file
        ...
        for record in SeqIO.parse(handle, 'fasta'): #Loop through the records in the input file
            if any([repre.replace('-', '').upper() in record.id.upper() for repre in representatives]): #If the records are in representative strains
    ...
    with open(f'{inpath}/{file.replace("faa", "fna")}') as handle_fna: #Same as above, but for nucleotide sequences
        ...
        for record in SeqIO.parse(handle_fna, 'fasta'):
            if any([repre.replace('-', '').upper() in record.id.upper() for repre in representatives]):
                print(record.id)
